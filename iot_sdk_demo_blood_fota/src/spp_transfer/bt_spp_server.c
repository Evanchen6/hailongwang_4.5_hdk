...
void spp_server_register(spps_receive_data_cb_t *callback)
{
...
bt_sdps_record_t* get_spp_sdp_record()
{
...
void spp_server_receive_data(bt_spp_data_received_ind_t* recv_ind_p)
{
    memcpy((void *) (spp_server_cntx_p->rx_data_buf + spp_server_cntx_p->rx_data_length), recv_ind_p->packet, recv_ind_p->packet_length);
    spp_server_cntx_p->rx_data_length += recv_ind_p->packet_length;
#ifdef RECEIVE_DATA_ENABLE
    if (recev_data_cb) {
        recev_data_cb(recv_ind_p->packet_length, recv_ind_p->packet);
    }
#endif
    return;
}
...
void spp_server_event_handler(bt_msg_type_t msg, bt_status_t status, void *param)
{
     ...
     switch (msg) {
        ...
        case BT_SPP_DATA_RECEIVED_IND:/*0x34000003*/
        {
            ...
            spp_server_receive_data((bt_spp_data_received_ind_t *)param);
        }
        break;
