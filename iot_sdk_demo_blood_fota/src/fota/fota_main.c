...
static void fota_recv_data_pack(bt_notify_event_data_t *event_data)
{
    ...
    if (mem_info_ptr->write_ptr >= mem_info_ptr->start_address
        ...
        if (mem_info_ptr->write_ptr + event_data->length < mem_info_ptr->end_address) {
			...
			LOGI("[FOTA] write data magic_ver = 0x%X \r\n", ((fota_entry_info_t *)event_data->data)->m_magic_ver);
...
void fota_reboot_device(uint32_t delay_msec)
{
...
void fota_btnotify_msg_hdlr(void *data)
{
    ...
    switch (p_data->evt_id) {
        case BT_NOTIFY_EVENT_CONNECTION: {
			...
			LOGE("[FOTA] read flash addr = 0x%x\r\n", FOTA_RESERVED_BASE + FOTA_RESERVED_LENGTH - BL_FOTA_UPDATE_INFO_RESERVE_SIZE - BL_BASE);
            for (int i = 0; i < UPDATE_INFO_SIZE; i++) {
				LOGE("[FOTA] read flash data[%d] = %d\r\n", i, update_info[i]);
            ...
            if (updated == true) {
				LOGE("[FOTA] erase flash addr = 0x%x\r\n", FOTA_RESERVED_BASE + FOTA_RESERVED_LENGTH - FLASH_BLOCK_SIZE - BL_BASE);
				LOGE("[FOTA] mem block_type = %d\r\n", s_fota_mem_info.block_type);
