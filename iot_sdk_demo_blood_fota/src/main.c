...
LOG_CONTROL_BLOCK_DECLARE(bt_spp_example);
...
//    &LOG_CONTROL_BLOCK_SYMBOL(bt_audio),
    ...
    &LOG_CONTROL_BLOCK_SYMBOL(bt_spp_example),
#ifdef MTK_SMART_BATTERY_ENABLE
...
int main(void)
{
	...
	log_config_print_switch(BTHCI, DEBUG_LOG_ON);
	        log_config_print_level(BT, PRINT_LEVEL_DEBUG);

        log_config_print_level(BTSPP, PRINT_LEVEL_DEBUG);

        log_config_print_level(bt_spp_example, PRINT_LEVEL_DEBUG);
    ...
    if (fota_is_triggered() || fota_is_updating()) {
#ifdef FOTA_FS_ENABLE
        /*mount file system.*/
        ...
        ble_app_common_init();
		LOG_W(bmt_demo, "start to scheduler.\r\n");
    } else {
#if defined (MTK_PORT_SERVICE_ENABLE)
