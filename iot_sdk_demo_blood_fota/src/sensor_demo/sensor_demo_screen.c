...
//    demo_ui_register_touch_event_callback(sensor_pen_event_handler, NULL);
    ...
    if(s_bp_measure_status==3){
        ...
        snprintf(s_bp_str, 32, "%ld/%ld mmHg \r\n", (int32_t)(tmp_bp_sbp), (int32_t)(tmp_bp_dbp));
		//if (tmp_bp_sbp <= 0 || tmp_bp_dbp <= 0) {
            //memset(s_bp_err, 0, 32);
		    //snprintf(s_bp_err, 32, "err:%ld, %ld, %ld\r\n", (int32_t)tmp_case1_error,(int32_t)tmp_case2_error,(int32_t)tmp_case3_error);
            sensor_string_info.x = LEFT_GAP;
            sensor_string_info.y = ITEM_HEIGHT + sensor_screen_cntx.hrv_value_y;
            sensor_string_info.string = sensor_convert_string_to_wstring(s_bp_err);
            sensor_string_info.length = strlen(s_bp_err);
            gdi_font_engine_display_string(&sensor_string_info);
		//}
...
void show_sensor_blood_pressure_screen(void)
{
 #ifdef FUSION_HEART_RATE_VARIABILITY_USE
     s_hrv_measure_status = 2;
     SENSOR_DEMO_LOGI("press HRV\r\n");
